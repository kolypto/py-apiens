...
def test_columns(ssn: sa.orm.Session):
    ...
    ssn.commit()  # make sure we start anew
...
def ssn(connection: sa.engine.Connection) -> sa.orm.Session:
    """ Provide: a database connection """
    with created_tables(connection, Base):

        ssn = sa.orm.Session(bind=connection, autocommit=False, autoflush=False)
        try:
            yield ssn
        finally:
            ssn.close()
