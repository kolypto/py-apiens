...
def test_decomarkers_simple():
    """ Test: is_decorated(), isinstance(), all_decorated_from() """
    # Create a class
    class A:
        @decomarker_1(1)
        def a(self): pass
...
def test_decomarkers_mixed_with_other_decorators():
    """ Test: how decomarker interacts with other decorators """
...
def test_several_decomarkers_at_the_same_time():
    """ Test: decomarkers do not interfere with one another """
    class C:
        @nop_decorator
        @decomarker_1(1)
        @decomarker_2(2)
...
class decomarker_1(decomarker):
    """ Example decomarker: with one argument """
    arg1: str
...
def nop_decorator(f):
    """ Decorator that wraps a function but does nothing """
    @wraps(f)
    def wrapper(*a, **k):
        return f(*a, **k)
    return wrapper
