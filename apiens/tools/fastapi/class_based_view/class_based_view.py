...
class class_based_view:
    ...
    def __init__(self, router: APIRouter, *routes: Union[Sequence, Route]):
    ...
    def _find_routes_on_class(self, ViewCls: type):
        ...
        self.routes.extend(
            decorated_endpoint.route  # type: ignore[attr-defined]
    ...
    def _patch_endpoint(self, route: Route):
        ...
        assert route.view_func is not None
        assert route.view_name is not None
        assert route.view_func_implemented is not None
        ...
        route.view_func = patch_method_fastapi_compatible(route.ViewCls, route.view_func)  # type: ignore[type-var,arg-type]
...
class api_route(decomarker):
    ...
    def __init__(self, path: str, methods: Union[str, Iterable[str]], **api_route_kwargs):
        ...
        """
        super().__init__()
        self.route = Route('...will-be-set-later...', path, methods, **api_route_kwargs)
...
class Route:
    ...
    # Internal fields
    ViewCls: Optional[type] = None
    view_func: Optional[Callable] = None
    view_func_implemented: Optional[bool] = None

    def add_endpoint_to_router(self, router: APIRouter):
        """ Add one route to the router """
        assert self.view_func is not None
